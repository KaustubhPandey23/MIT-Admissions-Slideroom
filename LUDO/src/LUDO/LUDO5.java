...
import LUDO.LUDO5.Dice;
import LUDO.LUDO5.Position;
import LUDO.LUDO5.Player;
import LUDO.LUDO5.Player.Piece;

public class LUDO5 extends JPanel implements MouseListener {
	...
	public LUDO5(boolean capture) {
		this(capture, true);
	}

	public LUDO5(boolean capture, boolean limsix) {
	...
	public LUDO5(int sizeX, int sizeY, Color a, Color b, Color c, Color d, boolean capture, boolean limsix) {
	...
	public void drawPiece(Graphics g, int player, int piece) {
		if (!players[player].pieces[piece].overlapDraw)
			return;
		Position p = null;
	...
	public void setOnStart(int player, int piece) {
		...
		players[player].pieces[piece].overlapDraw = true;
	...
	public class Player {
		...
		public class Piece {
			...
			public BufferedImage pieceImage() {
				...
				if (getState() == STATE_INT_OVERLAP && overlaps > 0) {
					if (this.color.equals(Color.WHITE))
						g.setColor(Color.BLACK);
					else
						g.setColor(Color.WHITE);
					int x = 0, y = 0;
					if (overlappingState == Piece.OVERLAP_INT_1_1) {
						x = (pieceImage.getWidth() - pieceImageSizeX[2]) / 2;
						y = (pieceImage.getHeight() - pieceImageSizeY[2]) / 2;
					}
