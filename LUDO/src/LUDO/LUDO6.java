...
import LUDO.LUDO6.Dice;
import LUDO.LUDO6.Position;
import LUDO.LUDO6.Player;
import LUDO.LUDO6.Player.Piece;

public class LUDO6 extends JPanel implements MouseListener {
	...
	public LUDO6(boolean capture) {
		this(capture, true);
	}

	public LUDO6(boolean capture, boolean limsix) {
	...
	public LUDO6(int sizeX, int sizeY, Color a, Color b, Color c, Color d, boolean capture, boolean limsix) {
	...
	public class Player {
		...
		public class Piece {
			...
			public BufferedImage pieceImage() {
				...
				int p = state + (check ? (test ? 3 : 0) : (test ? 2 : -1));
				...
				if (getState() == STATE_INT_OVERLAP && overlaps > 0) {
					...
					g.setFont(new Font("Arial", Font.PLAIN,
							(int) (((Toolkit.getDefaultToolkit().getScreenSize().height / 3 / 6 - 2) / 4 - 2) * 1.5)));
