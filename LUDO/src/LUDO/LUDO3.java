...
import LUDO.LUDO3.Dice;
import LUDO.LUDO3.Position;
import LUDO.LUDO3.Player;
import LUDO.LUDO3.Player.Piece;

public class LUDO3 extends JPanel implements MouseListener {
	public static final int maxPlace = 56;
	...
	public LUDO3(boolean capture) {
		this(capture, true);
	}

	public LUDO3(boolean capture, boolean limsix) {
	...
	public LUDO3(int sizeX, int sizeY, Color a, Color b, Color c, Color d, boolean capture, boolean limsix) {
	...
	public BufferedImage boardImage() {
		...
		for (int i = 0; i < coloured.length; i++)
			colored.add(coloured[i]);

		for (int i = 0; i < players.length; i++)
			for (int j = 0; j < maxPlace; j++) {
				boolean test = reverseCoordsPos.contains(players[i].route[j]);
				boolean test2 = colored.contains(j);
				Color color = test2 ? players[i].playerColor : Color.WHITE;
				g.setColor(color);
				g.fillRect(imagePosition.positions[players[i].route[j]].x - 1,
						imagePosition.positions[players[i].route[j]].y - 1, startPlaceSizeX / (test ? 6 : 3),
						startPlaceSizeY / (test ? 3 : 6));
				g.setColor(Color.LIGHT_GRAY);
				g.drawRect(imagePosition.positions[players[i].route[j]].x - 1,
						imagePosition.positions[players[i].route[j]].y - 1, startPlaceSizeX / (test ? 6 : 3),
						startPlaceSizeY / (test ? 3 : 6));
			}
	...
	public void checkPlaces(Graphics g, Color color) {
		if (checker == maxPlace * players.length)
			System.exit(0);
		draw(g);
		int i = checker / maxPlace, j = checker % maxPlace;
		checkPlace(g, i, j, color);
		try {
			Thread.sleep(1000);
		} catch (Exception e) {
	...
	public void checkPlace(Graphics g, int player, int place, Color color) {
		if (color == null)
			color = Color.CYAN;
		g.setColor(color);
		boolean check = reverseCoordsPos.contains(players[player].route[place]);
		g.fillRect(imagePosition.positions[players[player].route[place]].x - 1,
				imagePosition.positions[players[player].route[place]].y - 1, startPlaceSizeX / (check ? 6 : 3),
				startPlaceSizeY / (check ? 3 : 6));
	}
