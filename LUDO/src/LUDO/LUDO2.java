...
import LUDO.LUDO2.Dice;
import LUDO.LUDO2.Position;
import LUDO.LUDO2.Player;
import LUDO.LUDO2.Player.Piece;

public class LUDO2 extends JPanel implements MouseListener {
	...
	public LUDO2(boolean capture) {
		this(capture, true);
	}

	public LUDO2(boolean capture, boolean limsix) {
	...
	public LUDO2(int sizeX, int sizeY, Color a, Color b, Color c, Color d, boolean capture, boolean limsix) {
	...
	public BufferedImage boardImage() {
		...
		for (int i = 0; i < players.length; i++)
			for (int j = 0; j < coloured.length; j++) {
				...
				g.fillRect(imagePosition.positions[players[i].route[coloured[j]]].x - 1,
						imagePosition.positions[players[i].route[coloured[j]]].y - 1, startPlaceSizeX / (test ? 6 : 3),
